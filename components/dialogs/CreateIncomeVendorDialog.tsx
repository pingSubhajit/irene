...
import {LogoResult, searchLogos} from '@/lib/expenseVendor.methods'
import {useEffect, useState} from 'react'
import {cn, getInitialsFromName} from '@/lib/utils'
import {Avatar, AvatarFallback, AvatarImage} from '@/components/ui/avatar'
import {addIncomeVendorToDB} from '@/lib/incomeVendor.methods'
...
const CreateIncomeVendorDialog = ({open, setOpen}: { open: boolean, setOpen: (isOpen: boolean) => void }) => {
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		try {
			...
			const vendor = (await addIncomeVendorToDB({
				...values,
				userId: user!.id
			}))[0]
		...
		<Credenza open={open} onOpenChange={setOpen}>
			<CredenzaContent>
				<CredenzaHeader>
					...
					<CredenzaDescription>
						Create a new vendor to associate with your incomes.
					</CredenzaDescription>
				</CredenzaHeader>
				<CredenzaBody className="mt-10">
					<Form {...form}>
						<form onSubmit={form.handleSubmit(onSubmit)}>
							...
							<Button type="submit" size="lg" className="w-full text-lg mt-8" disabled={form.formState.isSubmitting}>
...
export default CreateIncomeVendorDialog
